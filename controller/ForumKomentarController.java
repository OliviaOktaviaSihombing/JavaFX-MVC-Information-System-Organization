...
import org.kmsi.dao.impl.KomentarDaoImpl;
import org.kmsi.model.Komentar;

public class ForumKomentarController implements Initializable {

    @FXML
    private ImageView imageViewLogo;
    @FXML
    private Label labelPengirim;
    @FXML
    private Label labelTopik;
    @FXML
    private TextArea isiKomentarTA;
    @FXML
    private TextArea komentariTA;
    @FXML
    ...
    private KomentarDao kDao;
    private boolean animasiButtonBool;

    public ForumKomentarController() {
        aDao = new AkunDaoImpl();
        fDao = new ForumDaoImpl(); 
        kDao = new KomentarDaoImpl();
    }

    public void loadKomentar() {
        List<String> komentar = kDao.getKomentarForum(fDao.getId());
        isiKomentarTA.setText("");
        for (int i = 0; i < komentar.size(); i++) {
            isiKomentarTA.appendText(komentar.get(i) + "\n\n");
        }
    }
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        labelPengirim.setText(fDao.getPengirim());
        labelTopik.setText(fDao.getTopik());
        loadKomentar();
    }
    ...
    @FXML
    private void komentar(ActionEvent event) {
        int idForum = fDao.getId();
        String isiKomentar = komentariTA.getText();
        String dari = linkUser.getText();
        Komentar komentar = new Komentar(idForum, isiKomentar, dari);
        if (!komentariTA.getText().equalsIgnoreCase("")) {
            kDao.saveKomentar(komentar);
        }
        fDao.incrementBanyakKomentar();
        komentariTA.setText("");
        loadKomentar();
    }
